    ...
    switch ($act) {
        ...
        case 'get_conversation_user':
            if (isset($_GET['shop_id']) && $_GET['shop_id']) {
                $classConversation = new Conversation();
                $page = isset($_GET['page']) ? $_GET['page'] : 1;
                $limit = isset($_GET['limit']) ? $_GET['limit'] : 10;
                $file = isset($_FILES['media']) ? $_FILES['media'] : null;
                echo json_encode($classConversation->get_conversation_user($_GET['shop_id'], $page, $limit), JSON_PRETTY_PRINT);
                return;
            } else {
                echo json_encode(['status' => false, 'message' => "Missing123"], JSON_PRETTY_PRINT);
                return;
            }
        case 'create_message_user':
            if (isset($_POST['shop_id']) && $_POST['shop_id']) {
                $classConversation = new Conversation();
                $message = isset($_POST['message_text']) ? ($_POST['message_text']) : "";
                $file = isset($_FILES['media']) ? $_FILES['media'] : null;
                echo json_encode($classConversation->crete_message_user($_POST['shop_id'], $message, $file), JSON_PRETTY_PRINT);
                return;
            } else {
                echo json_encode(['status' => false, 'message' => "Missing123"], JSON_PRETTY_PRINT);
                return;
        ...
        // shop message
        case 'create_message_shop':
            if (isset($_POST['user_id'])) {
                $classConversation = new Conversation();
                $message = isset($_POST['message_text']) ? ($_POST['message_text']) : "";
                $file = isset($_FILES['media']) ? $_FILES['media'] : null;
                echo json_encode($classConversation->create_message_shop($_POST['user_id'], $message, $file), JSON_PRETTY_PRINT);
                return;
            } else {
                echo json_encode(['status' => false, 'message' => "Missing123"], JSON_PRETTY_PRINT);
                return;
            }
        case 'get_conversation_shop':
            if (isset($_GET['user_id']) && $_GET['user_id']) {
                $classConversation = new Conversation();
                $page = isset($_GET['page']) ? $_GET['page'] : 1;
                $limit = isset($_GET['limit']) ? $_GET['limit'] : 10;
                $file = isset($_FILES['media']) ? $_FILES['media'] : null;
                echo json_encode($classConversation->get_conversation_shop($_GET['user_id'], $page, $limit), JSON_PRETTY_PRINT);
                return;
            } else {
                echo json_encode(['status' => false, 'message' => "Missing123"], JSON_PRETTY_PRINT);
                return;
            }
        case 'get_list_conversation_shop':
            $classConversation = new Conversation();
            echo json_encode($classConversation->get_list_conversation(), JSON_PRETTY_PRINT);
            break;
        default:
            break;
    }
