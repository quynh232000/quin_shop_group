...
<body>
  ...
  <div class="register ">
    <div class="wrapper">
      <form class="form-login" action="?mod=profile&act=forgotpassword<?=(isset($_GET['token'])?('&token='.$_GET['token']):"")?>" method="POST">
        <h1>Quên mật khẩu!</h1>
        <!-- invalidate -->
        <!-- form input email -->
        <?php
        if ($active == 'default') { ?>
          ...
          <div style="font-size:1.4rem;padding-bottom:18px" >Hoặc bằng 
          <a style="color:var(--primary)" href="?mod=profile&act=changepass_phone">Số điện thoại</a></div>
        <?php
        }
        ?>
