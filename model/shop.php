...
class Shop
{
    ...
    public function update_shop(
        ...
        $this->db->update("UPDATE user set user.role ='Seller' WHERE user.id = '$user_id' ");
...
select p.shop_id shop_id, c.name category_name from product p cross join category c where shop_id = '$id' order by  p.quantity_sold desc limit 4")->fetchAll();
    ...
    public function get_category_menus_shop($id)
    {
        ...
        // $set_sql = "SET sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));";
        // $query = " $query ="select *,id as category_id from
        // category 
        // WHERE id in 
        // (
        // select distinct (WITH RECURSIVE category_path AS (
        //     SELECT id, name, parent_id
        //     FROM category
        //     WHERE id = product.category_id -- Specify the ID of the child category
        //     UNION ALL
        //     SELECT c.id, c.name, c.parent_id
        //     FROM category_path cp
        //     JOIN category c ON cp.parent_id = c.id
        // )
        // SELECT id
        // FROM category_path
        // WHERE parent_id = 0) as id 
        // from product 
        // where shop_id ='$id' 
        // );
        // $this->db->select($set_sql);
        $query ="select *,id as category_id from
        category 
        WHERE id in 
        (
        select distinct (WITH RECURSIVE category_path AS (
