...
class UserAdmin
{
    ...
    public function selectUsers($role = "", $limit = "", $page = "", $uid = "")
    {
        ...
        if (!empty($uid)) {
            ...
            if (!empty($role) && $role == "All") {
                ...
                $userDeleted .= " WHERE u.is_deleted = 0 ";
            } else {
                $roleParam .= " WHERE u.role = '$role'";
                $paginate .= " limit $limit offset $offset";
                $userDeleted .= " AND u.is_deleted = 0 ";
            }
        }
        ...
        $query = "SELECT u.id as user_id,
                    ...
                    FROM quingroup.user u $whereUID $roleParam $userDeleted order by u.created_at desc $paginate;";
    ...
    {
        if ((Session::check_AdminAll())->status == false) {
            return Session::check_AdminAll();
        }
