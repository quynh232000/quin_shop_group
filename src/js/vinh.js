...
function getProductsByCate(data) {
  ...
  $.ajax({
    ...
    const html = data?.result
      .map((item) => {
        let numb = (item.price * (100 / (100 - item.percent_sale))) / 1000;
        let price = Math.round(numb) * 1000;
        ...
        if (item.pro_avg_level) {
          ...
          star = '<i style="color:gray" class="fa-regular fa-star"></i>'.repeat(
            5
          );
          star += "<span>(0)</span>";
        }
        ...
        return `
                                    ...
                                    <div class="product ">
                                        <div class="product-wrapper">
                                            <a href="?mod=page&act=home&product=${
                                              item.slug
                                            }" class="product-info">
                                            ...
                                            <div class="product-price">
                                                <div class="product-price-sale">đ${
                                                  item.price
                                                }</div>
                                                ${
                                                  item.percent_sale > 0
                                            ...
                                            <div class="product-btn" onclick="update_cart_user('plus','${
                                              item.id
                                            }',1)">
    ...
    for (let index = 0; index < Math.ceil(data.total / 8); index++) {
      ...
      // ${ disable }
      html_pagi += `<div class="g-nav-btn pagination_number ${active}" data="${
        index + 1
      }">${index + 1}</div>`;
    }
...
$(".cate_input").change(function (e) {
  ...
  $(".g-nav-left input:radio").each(function () {
