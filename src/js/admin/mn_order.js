...
function timeoutLocal() {
    if (localStorage.getItem("OBI_OD")) {
        ...
        }, 264000); // 4.4 minutes after last refresh page
    }
}

function timeoutLocalOrder() {
    if (localStorage.getItem("O_L")) {
        localStorage.removeItem("O_L")
    }
    const orderIdExists = checkStateOfData("order-status", "O_L", "All")
    if (!orderIdExists) {
        $(".render-tab-layout").html(loadingHTML)
    ...
    if (dataListOrders.length > 0) {
            ...
            html += `
                        <tr data-id-order="${el.order_id}" data-toggle="collapse" data-target="#demo${index}" class="accordion-toggle order-row-clickable">
                            <td class="py-1">
                                ...
                                ${el.shop_name}
                                    <img style="cursor: pointer;" class="shop-owner" src="assest/upload/${el.shop_icon}" alt="">
                                </div>
                            </td>
                            ...
                            <td>
                                <a href="?mod=admin&act=mn_user_detail&uid=${el.buyer_id}">
                                    <div class="d-flex flex-column gap-2 py-2">${el.buyer_name}
                                        <img style="cursor: pointer;" class="shop-owner" src="assest/upload/${el.buyer_avatar}" alt="">
                                    </div>
                                </a>
                            </td>
        ...
        html += `
            ...
            <ul class="pagination pagi">
                ...
                <li data-page-status="${statusKey}" onmousedown="return false" class="page-item ${currentPage === 1 ? " disabled " : ""} num-pagi-previous" style="cursor: ${currentPage == 1 ? "not-allowed" : "pointer"};"> <!-- disabled | style="cursor: not-allowed;"-->
        ...
        html += `
                <li data-page-status="${statusKey}" onmousedown="return false" class="page-item ${currentPage === totalPages ? " disabled" : ""} num-pagi-next" style="cursor: ${currentPage == totalPages ? "not-allowed" : "pointer"};">
